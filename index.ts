...
export interface SyncPass<Env, Ret> {
	run(input: Env): Ret;
}

/** An asynchronous pass: `run` returns a thenable/Promise. */
export interface AsyncPass<Env, Ret> {
	run(input: Env): Thenable<Ret>;
}

// Alias: a `Pass` may be synchronous or asynchronous.
export type Pass<Env, Ret> = SyncPass<Env, Ret> | AsyncPass<Env, Ret>;

export class PipelineBuilder<Input extends PlainObject, Env extends PlainObject = Input, Async extends true | false = false> {
	...
	private passes: Array<(env: PlainObject) => MaybePromise<PlainObject>> = [];
	 ...
	 */
	addPass<NewFields extends PlainObject, Ret extends MaybePromise<NewFields> = MaybePromise<NewFields>>(
		fn: ((env: Env) => Ret) | SyncPass<Env, Ret> | AsyncPass<Env, Ret>
	): PipelineBuilder<
		Input,
		Env & Awaited<Ret>,
	...
	> {
		...
		const wrapper = (env: PlainObject) => {
			if (typeof fn === 'function') return (fn as (env: Env) => Ret)(env as Env) as MaybePromise<PlainObject>;
			const maybeObj = fn as Pass<Env, Ret>;
			if (maybeObj && typeof (maybeObj as any).run === 'function') return (maybeObj as any).run(env as Env) as MaybePromise<PlainObject>;
			// Throw if the provided value is neither a function nor a pass.
			throw new TypeError('addPass expects a function or an object with a run(env) method');
		};

		this.passes.push(wrapper as (env: PlainObject) => MaybePromise<PlainObject>);
	...
	build(this: PipelineBuilder<Input, Env, true>): (input: Input) => Promise<Env>;
	build(this: PipelineBuilder<Input, Env, boolean>): (input: Input) => Env | Promise<Env>;
	build(): (input: Input) => Env | Promise<Env> {
		const passes = this.passes.slice();

		// Detects thenables/Promises by checking a `.then` function.
		const isThenable = (v: any): v is Promise<any> | { then: Function } => !!v && (typeof v === 'object' || typeof v === 'function') && typeof v.then === 'function';
			...
			for (let i = 0; i < passes.length; i++) {
				...
				if (isThenable(out)) {
					...
					return Promise.resolve(out).then((resolvedOut) => {
						...
						return remaining.reduce<Promise<PlainObject>>((p, next) => {
							return p.then((_env) => {
								const maybe = next(_env as Env);
								return Promise.resolve(maybe).then((r) => ({ ..._env, ...(r || {}) }));
							});
						}, Promise.resolve(env)).then((finalEnv) => finalEnv as Env);
					});
				}
